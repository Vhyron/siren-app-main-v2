...
const Header: React.FC<HeaderProps> = ({ user }) => {
  ...
  const handleReportMapPress = async () => {
    try {
      ...
      if (reportId) {
        handlePress({
          pathname: "/user/waitingResponder",
          params: { userId, reportId: String(reportId) },
        });
      } else {
      ...
      >
        ...
        >
          ...
          <TouchableOpacity
            ...
            onPress={() => handlePress("/user/call_dial")}
          >
          ...
          <TouchableOpacity
            style={styles.sliderNavItem}
            onPress={() => handlePress("/user/view_alert")}
          >
...
const styles = ScaledSheet.create({
  ...
  iconContainer: {
    position: "relative", // Allows positioning the red dot relative to the icon
  },
  redDot: {
    position: "absolute",
    top: 0, // Adjust to position correctly over the icon
    right: 0, // Adjust based on icon alignment
    width: 10,
    height: 10,

...
const MessagingItem = () => {
  ...
  const createNotification = async (receiverId: string, message: string) => {
    try {
      // Get sender's (current user's) data
      ...
      // Create notification under the receiver's notifications
      const notificationsRef = ref(db, `notifications/messages/${receiverId}`);
      const newNotification = {
        createdAt: Date.now(),
        message: `${senderName} sent: ${message.substring(0, 50)}${
          message.length > 50 ? "..." : ""
        }`,
  ...
  const createMessage = async () => {
    if (message.trim()) {
      try {
        ...
        // Determine receiver ID based on the current user's role
        const receiverId =
          roomData.user1 === currentUserId
            ? roomData.user2 // If current user is user1, send to user2
            : roomData.user1; // If current user is user2, send to user1
  ...
  // Add a function to mark notifications as read when entering the chat
  useEffect(() => {
    const markNotificationsAsRead = async () => {
      try {
        ...
        // Get all notifications for this room
        ...
        if (snapshot.exists()) {
          ...
          Object.keys(snapshot.val()).forEach((notificationId) => {
            updates[
              `notifications/messages/${currentUserId}/${notificationId}/read`
            ] = true;
          });

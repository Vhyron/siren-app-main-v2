...
const Messaging = () => {
  ...
  interface Room {
    ...
    messages?: Record<
      string,
      { senderId: string; message: string; createdAt: number }
    >;
  }
  type ContactType = {
    id: string;
    ...
    try {
      ...
      if (snapshot.exists()) {
        ...
        const filteredUsers = Object.entries(data)
          .filter(
            ([, user]) => user.role === "user" || user.role === "responder"
          )
          .map(([userId, user]) => ({
            id: userId,
  ...
  const createRoom = async (selectedUser: ContactType) => {
    try {
      ...
      if (!userSnapshot.exists()) {
        console.error("Current user data not found in the database");
        return;
      }
      ...
      const notificationsRef = ref(db, `notifications/messages/${selectedUser.id}`);
      const newNotification = {
        senderId: currentUser,
        receiverId: selectedUser.id,
        roomId: newRoomKey,
        message: `${currentUserName} has connected with you. Say hi!`,
        createdAt: Date.now(),
